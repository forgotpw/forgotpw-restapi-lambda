...
class VerificationCodesService {
  ...
  async validateCode(code, userToken) {
    ...
    logger.debug(`Trying to retrieve verification code from Dynamodb for ${userToken}...`)
    let dynamoResponse = null
    try {
      dynamoResponse = await docClient.get(params).promise()
    }
    catch (err) {
      logger.warn(`Unable to read verification code from Dynamodb for ${userToken}, err: `, err)
    ...
    logger.debug(`Verification code ${code} is valid for ${userToken}`)
    return true
  }
...
module.exports = VerificationCodesService
