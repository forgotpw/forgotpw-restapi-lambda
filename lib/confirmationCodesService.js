...
class ConfirmationCodesService {
  ...
  async publishSendCodeEvent(phone, defaultCountryCode) {
    const userTokenService = new UserTokenService()
    const userToken = await userTokenService.getUserTokenFromRawPhone(phone, defaultCountryCode)
  ...
  async validateCode(code, phone, defaultCountryCode) {
    const userTokenService = new UserTokenService()
    const userToken = await userTokenService.getUserTokenFromRawPhone(phone, defaultCountryCode)
    ...
    logger.debug(`Trying to retrieve confirmation code ${code} from Dynamodb for ${userToken}...`)
